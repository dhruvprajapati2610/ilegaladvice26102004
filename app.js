...
app.get("/pow_sections", async (req, res) => {
  const chapter = parseInt(req.query.chapter);
  if (isNaN(chapter)) {
    return res.status(400).send("Invalid chapter number");
  }
...
app.get("/pow_section", async (req, res) => {
  const section_number = req.query.section_number;
...
app.get("/pow_sections/search", async (req, res) => {
  const searchTerm = req.query.search;
  if (!searchTerm) {
    return res.status(400).json({
      success: false,
...
app.get("/pow_sections/search/:chapterNumber", async (req, res) => {
  const searchTerm = req.query.search;
  const chapterNumber = req.params.chapterNumber;
  if (!searchTerm && searchTerm === " ") {
    return res.status(400).json({
...
app.get("/pow_sections/:chapterNumber", async (req, res) => {
  const chapterNumber = req.params.chapterNumber;
...
app.get("/bsa_sections", async (req, res) => {
  const chapter = parseInt(req.query.chapter);
  if (isNaN(chapter)) {
    return res.status(400).send("Invalid chapter number");
  }
...
app.get("/bsa_section", async (req, res) => {
  const section_number = req.query.section_number;
...
app.get("/bsa_sections/search", async (req, res) => {
  const searchTerm = req.query.search;
  if (!searchTerm) {
    return res.status(400).json({
      success: false,
...
app.get("/bsa_sections/search/:chapterNumber", async (req, res) => {
  const searchTerm = req.query.search;
  const chapterNumber = req.params.chapterNumber;
  if (!searchTerm && searchTerm === " ") {
    return res.status(400).json({
...
app.get("/bsa_sections/:chapterNumber", async (req, res) => {
  const chapterNumber = req.params.chapterNumber;
...
LIMIT 10;
    ...
    const posts = postsResult.rows;
    res.json({ success: true, initialPosts: posts });
  } catch (error) {
    console.error("Error Loading Initial Community Posts", error);
    res
      .status(500)
      .send({ success: false, message: "Error fetching initial Posts" });
  }
});
...
app.post("/community/posts", isuAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const previousPosts = req.body.posts;
    ...
    //Filter posts exluding the previous posts
    for (let i = posts.length - 1; i >= 0; i--) {
      if (
        previousPosts.some((prevPost) => prevPost.post_id === posts[i].post_id)
      ) {
        posts.splice(i, 1);
      }
    }
...
`,
    ...
    const clientResult = await sendEmailWithRetry(
      clientMailOptions,
      transporter
    );
    ...
    if (!clientResult.success) {
      responseMessage = { success: false, message: "Please Enter Valid Email" };
